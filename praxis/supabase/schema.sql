...
-- Student profile details (linked to Supabase Auth)
...
-- Student assignments and attempts power the student dashboard.
CREATE TABLE IF NOT EXISTS student_simulation_assignments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  student_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  simulation_id UUID NOT NULL REFERENCES simulations(id) ON DELETE CASCADE,
  assigned_by_professor_id UUID REFERENCES professors(id) ON DELETE SET NULL,
  due_date TIMESTAMPTZ,
  assigned_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(student_id, simulation_id, assigned_by_professor_id)
);

CREATE TABLE IF NOT EXISTS student_simulation_attempts (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  student_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  simulation_id UUID NOT NULL REFERENCES simulations(id) ON DELETE CASCADE,
  session_id UUID NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
  participant_id UUID REFERENCES participants(id) ON DELETE SET NULL,
  assignment_id UUID REFERENCES student_simulation_assignments(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_student_assignments_student ON student_simulation_assignments(student_id);
CREATE INDEX IF NOT EXISTS idx_student_assignments_simulation ON student_simulation_assignments(simulation_id);
CREATE INDEX IF NOT EXISTS idx_student_attempts_student ON student_simulation_attempts(student_id, status, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_student_attempts_simulation ON student_simulation_attempts(simulation_id);
CREATE INDEX IF NOT EXISTS idx_student_attempts_session ON student_simulation_attempts(session_id);
...
-- Students can maintain their own profile details
...
CREATE POLICY "Students can insert own student profile" ON student_profiles
...
-- Student assignments and attempts
DROP POLICY IF EXISTS "Students can view own assignments" ON student_simulation_assignments;
CREATE POLICY "Students can view own assignments" ON student_simulation_assignments
  FOR SELECT USING (auth.uid() = student_id);

DROP POLICY IF EXISTS "Professors can manage own student assignments" ON student_simulation_assignments;
CREATE POLICY "Professors can manage own student assignments" ON student_simulation_assignments
  FOR ALL USING (auth.uid() = assigned_by_professor_id)
  WITH CHECK (auth.uid() = assigned_by_professor_id);

DROP POLICY IF EXISTS "Students can view own attempts" ON student_simulation_attempts;
CREATE POLICY "Students can view own attempts" ON student_simulation_attempts
  FOR SELECT USING (auth.uid() = student_id);

DROP POLICY IF EXISTS "Students can insert own attempts" ON student_simulation_attempts;
CREATE POLICY "Students can insert own attempts" ON student_simulation_attempts
  FOR INSERT WITH CHECK (auth.uid() = student_id);

DROP POLICY IF EXISTS "Students can update own attempts" ON student_simulation_attempts;
CREATE POLICY "Students can update own attempts" ON student_simulation_attempts
  FOR UPDATE USING (auth.uid() = student_id) WITH CHECK (auth.uid() = student_id);

DROP POLICY IF EXISTS "Professors can view attempts for own simulations" ON student_simulation_attempts;
CREATE POLICY "Professors can view attempts for own simulations" ON student_simulation_attempts
  FOR SELECT USING (
    simulation_id IN (SELECT id FROM simulations WHERE professor_id = auth.uid())
  );
...
DROP TRIGGER IF EXISTS update_student_attempts_updated_at ON student_simulation_attempts;
CREATE TRIGGER update_student_attempts_updated_at
  BEFORE UPDATE ON student_simulation_attempts
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();
