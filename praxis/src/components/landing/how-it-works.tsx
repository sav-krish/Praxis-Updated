...
const howItWorksStepFourScreens = [
  ...
  },
  {
    ...
    width: 2272,
    height: 1278,
    alt: "Additional results view showing debrief analytics and response insights",
  },
] as const;

const howItWorksStepMasterBackgrounds = [
  "linear-gradient(120deg, #fff0e1 0%, #ffd0a8 100%)",
  "linear-gradient(120deg, #ffe7d2 0%, #fdb77f 100%)",
  "linear-gradient(to top, #fff2e6 0%, #ffdabb 100%)",
  "linear-gradient(135deg, #fff3e6 0%, #ffe5cf 52%, #ffd7b6 100%)",
] as const;
...
function HowItWorksCard({
  ...
  const screenshotShadow =
    "shadow-[0_26px_55px_-14px_rgba(128,52,20,0.28),0_12px_28px_-10px_rgba(128,52,20,0.14)]";
  ...
  return (
    <div
      className={
        single
          ? "mx-auto flex w-full max-w-[64rem] flex-col items-center justify-center overflow-hidden rounded-3xl p-3 shadow-[0_20px_50px_-20px_rgba(128,52,20,0.18)] ring-1 ring-white/50 sm:p-4 md:p-6 lg:p-8 relative aspect-[4/3] md:aspect-[16/9] lg:aspect-[21/9] max-h-[50vh] min-h-[250px] md:max-h-[60vh] lg:max-h-[70vh]"
          : "mx-auto flex w-full max-w-[64rem] flex-col items-center justify-center overflow-hidden rounded-3xl p-3 shadow-[0_20px_50px_-20px_rgba(128,52,20,0.18)] ring-1 ring-white/50 sm:p-4 md:p-6 lg:p-8 relative aspect-[4/3] md:aspect-[16/9] lg:aspect-[21/9] max-h-[50vh] min-h-[250px] md:max-h-[60vh] lg:max-h-[70vh]"
      }
      style={frameStyle}
    >
      <div
        className={
          single
            ? "flex h-full w-full items-center justify-center px-5 py-4 sm:px-8 sm:py-5 md:px-10 md:py-6"
        ...
        {screens.map((img) => {
          ...
          if (single) {
            return (
              <Image
                ...
                alt={img.alt}
                sizes={sizes}
                className={`mx-auto block h-auto w-auto max-w-[min(100%,52rem)] object-contain max-h-[78%] sm:max-h-[82%] ${screenshotRadius} ${screenshotShadow}`}
                draggable={false}
              />
            );
          }
          ...
          return (
            ...
            >
              ...
              {isOverlayTarget && (
                <div
                  className="pointer-events-none absolute inset-x-0 bottom-0 z-10 flex justify-center bg-gradient-to-t from-[#fff7ee]/95 from-20% via-[#fff7ee]/70 to-transparent px-3 pb-3 pt-12 sm:px-4 sm:pb-4 sm:pt-16"
                  aria-hidden="true"
                >
                  <div className="w-full max-w-[22rem] sm:max-w-[28rem] select-none mx-auto mt-auto">
                    <div className="flex min-w-0 flex-row gap-1.5 sm:gap-2.5">
                      ...
                      <div className="flex min-h-[36px] flex-1 items-center justify-center gap-1 rounded-md border-[1.5px] border-[#f5d3bd] bg-white/95 px-1.5 text-[10px] font-semibold leading-none tracking-tight text-[#4a1f10] shadow-md backdrop-blur-sm whitespace-nowrap sm:min-h-[40px] sm:gap-1.5 sm:rounded-lg sm:px-2 sm:text-[11px] md:text-xs">
                        <PenLine className="h-3 w-3 shrink-0 text-[#91593a] sm:h-3.5 sm:w-3.5" />
                        Create Manually
                      </div>
                    </div>
                  </div>
    ...
    case 1:
      return (
        <HowItWorksCard
          ...
          createButtonsOverlaySrc="/new_simulation_final2.png"
        />
      );
    case 2:
      return (
