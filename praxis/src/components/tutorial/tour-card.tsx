...
export function TourCard({
  ...
  return (
    <div className="w-[340px] max-w-[calc(100vw-32px)] rounded-2xl border border-border bg-white p-5 shadow-[0_18px_50px_rgba(15,36,71,0.25)]">
      <div className="mb-2 flex items-start justify-between gap-2">
        <span className="inline-flex items-center gap-1.5 text-xs font-medium text-[#5BA2D8]">
        ...
        {skipTour && (
          <button
            ...
            className="rounded-full p-1 text-muted-text transition-colors hover:bg-[#EAF1FF] hover:text-ink"
          >
            <X className="h-4 w-4" />
          </button>
        )}
          ...
          <div className="flex items-center gap-2">
            {!isFirst && (
              <button
                type="button"
                onClick={prevStep}
                className="inline-flex items-center gap-1 rounded-md border border-border bg-white px-2.5 py-2 text-xs font-medium text-ink transition-colors hover:bg-[#F4F8FE]"
            ...
            <button
              type="button"
              onClick={nextStep}
              className="inline-flex items-center gap-1.5 rounded-md bg-[#5BA2D8] px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-[#4f93c8]"
            >
              {isLast ? "Finish" : "Next"}
