...
import { Heart, Search, Clock, Trophy, Loader2, ExternalLink } from "lucide-react";
...
export function LibraryView({
  ...
  const [usingSimId, setUsingSimId] = useState<string | null>(null);
  ...
  const SimCard = ({ sim }: { sim: SimulationCard }) => {
    ...
    const usingSim = usingSimId === sim.id;

    const handleUseSimulation = async (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      setUsingSimId(sim.id);
